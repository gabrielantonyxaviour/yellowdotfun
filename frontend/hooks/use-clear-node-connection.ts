...
export const useClearNodeConnection = () => {
  ...
  const messageSigner = useCallback(async (payload: any) => {
    ...
    const { serialized: signature } = stateWalletRef.current.signingKey.sign(messageBytes)
    return signature
  }, [])

  const connect = useCallback(async (clearNodeUrl: string, stateWallet: any) => {
    ...
    ws.onopen = async () => {
      ...
      try {
        ...
        setError(`Auth request failed: ${err instanceof Error ? err.message : 'Unknown error'}`)
      }
    }
  ...
  const getLedgerBalances = useCallback(async (participant: string) => {
